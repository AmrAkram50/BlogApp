    ...
    <li className='relative group'>
      {item.type === "link" ? (
        ...
        <div className='flex flex-col items-center'>
          <button className='flex gap-x-1 items-center px-4 py-2' onClick={toggleDropdownHandler}>
            <span>{item.name}</span>
            <MdKeyboardArrowDown />
          </button>
  ...
  return (
    <section className='sticky top-0 left-0 right-0 z-50 bg-white'>
      <header className='container mx-auto px-5 flex justify-between py-4 items-center'>
        ...
        flex transition-all duration-300 z-[49] mt-[56px] lg:mt-0 bg-dark-hard lg:bg-transparent flex-col w-full lg:w-auto justify-center lg:flex-row lg:justify-end fixed top-0 bottom-0 lg:static gap-x-9 items-center`}>
          <ul className='flex flex-col items-center gap-y-5 text-white lg:text-dark-soft lg:flex-row gap-x-2 font-semibold'>
            {navItemInfo.map((item) => (
              <NavItem key={item.name} item={item} />
            ))}
          </ul>
          <button className='mt-5 lg:mt-0 border-2 border-blue-500 text-blue-500 font-semibold px-6 py-2 
          rounded-full hover:bg-blue-500 hover:text-white transition-all duration-300'>Sign In</button>
        </div>
      </header>
    </section>
  )
